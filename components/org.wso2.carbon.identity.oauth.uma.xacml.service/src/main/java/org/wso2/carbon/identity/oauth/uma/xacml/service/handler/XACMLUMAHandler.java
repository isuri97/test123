...
public class XACMLUMAHandler {
     ...
     */
    public boolean isAuthorized(List<Resource> resource, String clientId) throws IdentityOAuth2Exception {
    ...
    private RequestDTO createRequestDTO(List<Resource> resource, OAuthAppDO authApp) {

        String listString = "";
        List<RowDTO> rowDTOs = new ArrayList<>();
        for (Resource rei : resource) {

            for (String scopes : rei.getResourceScopes()) {
                listString += scopes + ",";
            }
            RowDTO actionDTO = createRowDTO(listString, XACMLAppUMAConstants.AUTH_ACTION_ID,
                    XACMLAppUMAConstants.ACTION_CATEGORY);
            RowDTO resourceDTO = createRowDTO(rei.getResourceId(), XACMLAppUMAConstants.RESOURCE_ID,
